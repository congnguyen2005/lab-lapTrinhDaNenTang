  ...
  const loadSavedCredentials = async () => {
    try {
      ...
      if (savedEmail && savedPassword) {
        setEmail(savedEmail);
        setPassword(CryptoJS.AES.decrypt(savedPassword, "secret_key").toString(CryptoJS.enc.Utf8));
        setRememberMe(true);
      }
    } catch (error) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (rememberMe) {
        ...
        await AsyncStorage.setItem("savedPassword", CryptoJS.AES.encrypt(password, "secret_key").toString());
      } else {
